...
**answer 1 回のコストは web 検索の約 10 倍です。** Brave は検索結果をモデルに入力トークンとして食わせて課金するため、`answer` 1 回の実測は入力約 10,000 トークン・$0.054〜0.058 でした。**回答言語に英語以外を指定すると引用（citations）が不安定です**（同じ質問で実測: `--lang en` は毎回返り、`--lang ja` は 3 回中 1 回）。英語以外を指定した `answer` に引用が無いときは `note` でその旨が付くので、出典が要るときは再試行するか `--lang en` で聞いてください。

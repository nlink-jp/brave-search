...
func (e *Engine) Answer(ctx context.Context, r AnswerRequest) (*AnswerResult, error) {
	...
	return shapeAnswer(p.Question, "answer", res, meta), nil
}
...
func (e *Engine) Research(ctx context.Context, r ResearchRequest, progress func(brave.Progress)) (*AnswerResult, error) {
	...
	// timeout, so a 300-second research call is not cut off at 30.
	client := e.Client.WithTimeout(e.Cfg.ResearchTimeout(p.MaxSeconds))
	res, meta, err := client.Answer(ctx, p)
	if err != nil {
		return nil, err
	}
	return shapeAnswer(p.Question, "research", res, meta), nil
}

func validateAnswerCommon(p brave.AnswerParams) error {
	if err := validateQuery(p.Question); err != nil {
		return err
	}
...
func shapeAnswer(question, mode string, res *brave.AnswerResult, meta *brave.Meta) *AnswerResult {
	out := &AnswerResult{
		Question:   question,
		Mode:       mode,
		Answer:     res.Text,

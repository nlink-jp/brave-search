...
## e2e: Live end-to-end tests against the real Brave Search API (network required)
e2e: build
...
## (brave-search-vX.Y.Z-<os>-<arch>.<ext>).
package: build-all
...
BREW_KIND := formula
BREW_DESC := Brave Search API (web, LLM context, answers) as a CLI and MCP server
